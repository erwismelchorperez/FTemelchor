...
class FunctionalTreeClassifier:
    ...
    def _best_function_split(self, X, y):
        X_encoded = pd.get_dummies(X)
        X_scaled = self.scaler.fit_transform(X_encoded)
        y_binary = (y == y.mode().iloc[0]).astype(int)

        clf = LinearRegression(max_iter=5000, solver='liblinear')
        clf.fit(X_scaled, y_binary)

        def split_func(row):
            row_encoded = pd.get_dummies(pd.DataFrame([row]))
            row_encoded = row_encoded.reindex(columns=clf.feature_names_in_, fill_value=0)
            row_scaled = self.scaler.transform(row_encoded)
            return clf.predict(row_scaled)[0] == 1

        return split_func, clf
...
if __name__ == "__main__":
    ...
    # Entrenar árbol
    clf = FunctionalTreeClassifier(max_depth=5)
